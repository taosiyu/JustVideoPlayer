...
typedef NS_ENUM(NSUInteger, JustVideoToolBoxErrorCode) {
    JustVideoToolBoxErrorCodeExtradataSize,
    JustVideoToolBoxErrorCodeExtradataData,
    JustVideoToolBoxErrorCodeCreateFormatDescription,
    JustVideoToolBoxErrorCodeCreateSession,
    JustVideoToolBoxErrorCodeNotH264,
};


//需要链接硬件解码 videoToolBox
@interface JustVideoToolBox()
{
...
@implementation JustVideoToolBox
...
- (NSError *)setupVTSession
{
    ...
    if (codec_id == AV_CODEC_ID_H264) {
        ...
        if (extradata_size < 7 || extradata == NULL) {
            error = [NSError errorWithDomain:@"extradata error" code:JustVideoToolBoxErrorCodeExtradataSize userInfo:nil];
            return error;
        }
        ...
        if (extradata[0] == 1) {
            ...
            if (self->_format_description == NULL) {
                error = [NSError errorWithDomain:@"create format description error" code:JustVideoToolBoxErrorCodeCreateFormatDescription userInfo:nil];
                return error;
            }
            ...
            if (status != noErr) {
                error = [NSError errorWithDomain:@"create session error" code:JustVideoToolBoxErrorCodeCreateSession userInfo:nil];
                return error;
            }
            ...
            error = [NSError errorWithDomain:@"deal extradata error" code:JustVideoToolBoxErrorCodeExtradataData userInfo:nil];
            return error;
        }
    } else {
        error = [NSError errorWithDomain:@"not h264 error" code:JustVideoToolBoxErrorCodeNotH264 userInfo:nil];
        return error;
    }
...
static void outputCallback(void * decompressionOutputRefCon, void * sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef imageBuffer, CMTime presentationTimeStamp, CMTime presentationDuration)
{
    @autoreleasepool
    {
        JustVideoToolBox * videoToolBox = (__bridge JustVideoToolBox *)decompressionOutputRefCon;
